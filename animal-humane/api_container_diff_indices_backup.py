#I'm not sure this isn't being overwritten with some previous version when I re-build the 
#container (docker-compose build api)
...
def run_diffs(handler):
    ...
    #print("About to print results", file=sys.stderr, flush=True)
    #try:
    #    results  # check if results exists
    #except NameError:
    #    results = {}
    #if not results:
    #    print(json.dumps({"status": "no_results"}), flush=True)
...
if __name__ == "__main__":
    ...
    print(f"results returned to main from run_diffs are: {results}")
