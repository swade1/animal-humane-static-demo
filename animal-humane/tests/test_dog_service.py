...
import sys
sys.path.append('/Users/swade/KIRO-project/professional-portfolio')
...
@pytest.mark.asyncio
async def test_get_length_of_stay_distribution(self, dog_service, mock_es_service):
    """Test retrieving length of stay distribution"""
    # Setup expected data
    mock_distribution = [
        {"range_start": 0, "range_end": 1, "count": 5},
        {"range_start": 1, "range_end": 2, "count": 3},
        {"range_start": 2, "range_end": 3, "count": 8}
    ]
    mock_es_service.get_length_of_stay_distribution.return_value = mock_distribution

    # Execute
    result = await dog_service.get_length_of_stay_distribution()

    # Verify
    assert result == mock_distribution
    mock_es_service.get_length_of_stay_distribution.assert_called_once_with(None)
